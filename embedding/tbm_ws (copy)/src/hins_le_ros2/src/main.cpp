...
int main(int argc, char *argv[]) 
{
  ...
  auto laser_pub = node->create_publisher<sensor_msgs::msg::LaserScan>("/scan", rclcpp::QoS(10));
  ...
  // Không dùng rate limiter - GetFullScan() tự block chờ data
  ...
  while (rclcpp::ok()) {
    ...
    scan_msg->angle_min = 0.0;                                                              // 测量范围的开始值
    scan_msg->angle_max = 2.0 * M_PI;                                                       // 测量范围的结束值
    scan_msg->angle_increment = 2.0 * M_PI/float(data.distance_data.size());                  // 角度增量
               ...
               ShadowsFilterParam &shadows_filter_param)
{
  ...
  node->declare_parameter("use_udp", false);
