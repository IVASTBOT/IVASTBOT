...
ScanData LaserDataReceiver::GetFullScan()
{
    ...
    if (scan_data_.size() >= 2 && IsConnected()) {
        // Take the latest completed scan (second-to-last), drop all older ones
    ...
    if (!error) {
      ...
      try {
        ...
        while (HandleNextPacket()) { }
          ...
          RCLCPP_WARN_SKIPFIRST_THROTTLE(rclcpp::get_logger("hins_le_node"),
              *rclcpp::Clock::make_shared(), 5000,
              "HandleSocketRead exception (recovering): %s", e.what());
      }
...
bool LaserDataReceiver::Connect()
{
    try {
        ...
        if(use_udp_){
            ...
            RCLCPP_INFO_STREAM(rclcpp::get_logger("hins_le_node"),"begin tcp async_connect.");
            tcp_socket_ptr_->async_connect(endpoint, boost::bind(&LaserDataReceiver::connect_handler, this, boost::asio::placeholders::error));
            RCLCPP_INFO_STREAM(rclcpp::get_logger("hins_le_node"),"tcp async_connect end.");
        }
        // 开始异步读取数据
        if(use_udp_){
            udp_socket_ptr_->async_receive(boost::asio::null_buffers(), boost::bind(&LaserDataReceiver::HandleSocketRead, this, boost::asio::placeholders::error));
...
void LaserDataReceiver::WriteBufferBack(char* src, std::size_t num_bytes)
{
    if (ring_buffer_.size() + num_bytes > ring_buffer_.capacity()) {
        // Buffer full — drop oldest data instead of crashing
        std::size_t overflow = ring_buffer_.size() + num_bytes - ring_buffer_.capacity();
        ring_buffer_.erase_begin(overflow);
    }
...
bool LaserDataReceiver::GetRangeData(int16_t head_index)
{
   bool ret = false;
    ...
    if (ring_buffer_.size() - head_index >= kXSPackageHeadSize) {
        ...
        // Dynamically update expected point count from actual scan data
        if (all_point_num > 0) {
            scan_all_point_num_ = all_point_num;
        }
        ...
        if ((ring_buffer_.size() - head_index) >= body_size) {
            ...
            for (int i = 0; i < header.data_size; i++) {
                ...
                int idx = begin_point_index + i;
                if (idx >= 0 && idx < (int)scan_data.distance_data.size()) {
                    scan_data.distance_data[idx] = distance;
                    scan_data.amplitude_data[idx] = intensity;
                } else {
                    RCLCPP_WARN_SKIPFIRST_THROTTLE(rclcpp::get_logger("hins_le_node"),
                        *rclcpp::Clock::make_shared(), 5000,
                        "Index out of range: idx=%d, size=%zu, begin_angle=%d, end_angle=%d",
                        idx, scan_data.distance_data.size(),
                        header.start_angle, header.end_angle);
                }
            }
            ...
            {                
                ...
                if (scan_data_.size() > 3) {
                    scan_data_.pop_front();
                    RCLCPP_WARN_SKIPFIRST_THROTTLE(rclcpp::get_logger("hins_le_node"),
                        *rclcpp::Clock::make_shared(), 5000,
                        "buffer data too many, drops it (suppressing for 5s)");
                }
