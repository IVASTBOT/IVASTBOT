...
void XingSongDriver::ShadowsFilter(ScanData& scan_data, int scan_num)
{
  ...
  for (int i = 0; i < scan_num; i += shadows_filter_param_.traverse_step)
  {
        ...
        search_index++)
    {
      ...
      int filter_idx = abs(search_index_tmp - i) - 1;
      if (filter_idx < 0 || filter_idx >= (int)shadows_filter_threshold_max_.size())
          continue;
      if( (a_b_rate < shadows_filter_threshold_min_[filter_idx]) ||                      // 如果存在拖影现象
          (a_b_rate > shadows_filter_threshold_max_[filter_idx])   )        
      {
