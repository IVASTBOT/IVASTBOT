...
private:
  ...
  /** Set acceleration / deceleration ramp time (register 0x0A).
   *  Units: multiples of 100 ms (1 = 100 ms, 0 = instant).
   *  accel_100ms: time to ramp 0 → 3000 RPM
   *  decel_100ms: time to ramp 3000 → 0 RPM */
  void motor_set_accel_time(uint8_t can_id, uint8_t accel_100ms, uint8_t decel_100ms);
  ...
  int16_t prev_rpm_left_ = 0;      // Track last sent RPM to avoid redundant CAN writes
  int16_t prev_rpm_right_ = 0;
  int read_cycle_count_ = 0;        // For encoder read throttling
  std::array<double, 2> dt_accum_ = {0.0, 0.0};  // Accumulated dt for missed encoder reads
  ...
  static constexpr uint8_t REG_ACCEL_TIME   = 0x0A;  // Accel/decel time (D_H=accel x100ms, D_L=decel x100ms)
