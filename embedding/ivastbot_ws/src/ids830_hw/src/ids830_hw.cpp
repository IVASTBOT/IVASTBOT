...
void IDS830HW::motor_set_accel_time(uint8_t can_id, uint8_t accel_100ms, uint8_t decel_100ms)
{
  // Register 0x0A: Data_H = accel time (×100ms, 0→3000RPM), Data_L = decel time (×100ms, 3000→0RPM)
  uint8_t data[8] = {
    GROUP_DEFAULT, FUNC_WRITE,
    REG_ACCEL_TIME,
    accel_100ms, decel_100ms,
    REG_EMPTY, 0x00, 0x00
  };
  slcan_send(can_id, data, 8);
  std::this_thread::sleep_for(std::chrono::milliseconds(50));
  RCLCPP_INFO(rclcpp::get_logger(LOG),
    "Motor CAN_ID=0x%02X: accel=%d*100ms, decel=%d*100ms",
    can_id, accel_100ms, decel_100ms);
}
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // Set acceleration/deceleration ramp to 300ms (factory default can be 500ms+ causing slow ramp;
  // values <300ms can stall cold motors / trip overcurrent on the IDS830)
  motor_set_accel_time(left_id_,  3, 3);
  motor_set_accel_time(right_id_, 3, 3);
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  // Read encoder every 2nd cycle (50Hz) to reduce CAN bus load
  // This leaves more bus time for speed commands and prevents congestion
  read_cycle_count_++;
  bool do_encoder_read = (read_cycle_count_ % 2 == 0);

  if (do_encoder_read) {
    // Request encoder data from both motors (non-blocking)
    motor_request_encoder(left_id_);
    motor_request_encoder(right_id_);
  }

  // Always try to parse any available encoder responses (from RX thread)
  ...
  // Use accumulated dt since last successful read for accurate velocity
  // --- Left motor ---
  if (left_ok) {
    ...
    hw_velocities_[0] = (dt_accum_[0] > 1e-4) ? rad_delta / dt_accum_[0] : 0.0;
    dt_accum_[0] = 0.0;
  } else {
    dt_accum_[0] += dt;
  }
  ...
  if (right_ok) {
    ...
    hw_velocities_[1] = (dt_accum_[1] > 1e-4) ? rad_delta / dt_accum_[1] : 0.0;
    dt_accum_[1] = 0.0;
  } else {
    dt_accum_[1] += dt;
  }
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  if (motors_idle_) {
    if (!both_zero) {
      motors_idle_ = false;
      settle_count_ = 0;
      prev_rpm_left_ = -9999;   // Force first send
      prev_rpm_right_ = -9999;
  ...
  if (both_zero) {
    ...
    if (settle_count_ <= SETTLE_THRESHOLD) {
      // Only send zero if we haven't already
      if (prev_rpm_left_ != 0 || prev_rpm_right_ != 0 || settle_count_ <= 3) {
        motor_set_speed(left_id_, 0);
        motor_set_speed(right_id_, 0);
        prev_rpm_left_ = 0;
        prev_rpm_right_ = 0;
    ...
    settle_count_ = 0;
    // Send both motors TOGETHER when either RPM changes
    // This prevents 1-cycle desync that causes the robot to veer
    if (rpm_left != prev_rpm_left_ || rpm_right != prev_rpm_right_) {
      motor_set_speed(left_id_, rpm_left);
      motor_set_speed(right_id_, rpm_right);
      prev_rpm_left_ = rpm_left;
      prev_rpm_right_ = rpm_right;
    }
  }
